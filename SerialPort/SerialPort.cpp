...
bool SerialRingBuffer::get(uint8_t* b) {
  buf_size_t t = tail_;
  if (head_ == t) {
    // hack for OpenLog to send XON when receive buffer is empty
    if (paused) {
        paused = false;
        while (!(*usart[0].ucsra & M_UDRE)) {}
        *usart[0].udr = XON;
        //while (!(*usart[0].ucsra & M_UDRE)) {}
        //*usart[0].udr = 'Q';
    }
...
SerialRingBuffer::buf_size_t SerialRingBuffer::get(uint8_t* b, buf_size_t n) {
  ...
  if (nr > n) nr = n;
    ...
    paused = false;
    while (!(*usart[0].ucsra & M_UDRE)) {}
    *usart[0].udr = XON;  // hack for OpenLog to send XON when receive buffer is empty
    //while (!(*usart[0].ucsra & M_UDRE)) {}
    //*usart[0].udr = 'Q';
  }
  memcpy(b, &buf_[t], nr);
...
bool SerialRingBuffer::put(uint8_t b) {
  ...
  // if buffer is nearly full, send XOFF
  int m = head_ - tail_;
  if (m < 0) m = m + size_;
  if (m >= (size_-8)) {
    while (!(*usart[0].ucsra & M_UDRE)) {}
